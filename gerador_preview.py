# gerador_preview.py
# Descrição: Versão final com todas as melhorias, incluindo sumário e paginação inteligente.
...
class GeradorHTMLPreview:
    ...
    def _adicionar_elemento(self, html_elemento, altura_estimada):
        ...
        # Regra anti-viúva/órfão para títulos
    ...
    def gerar_html(self) -> str:
        ...
        <style>
            ...
            .capa, .folha-rosto { text-align: center; height: 100%; }
            ...
            .sumario-item { list-style-type: none; text-indent: 0; }
            .sumario-item a { text-decoration: none; color: black; }
            .sumario-item a:hover { text-decoration: underline; }
            .sumario-nivel-2 { margin-left: 2em; }
            .sumario-nivel-3 { margin-left: 4em; }
        </style>
        """
        ...
        <div class="capa">
            <p><strong>{cfg.instituicao.upper()}</strong></p>
            <br><br><br>
            <p><strong>{autores_capa_html}</strong></p>
            <br><br><br><br>
            <p><strong>{self.doc_abnt.titulo.upper()}</strong></p>
            <div class="posicao-final-pagina">
                <p>{cfg.cidade.upper()}</p>
                <p>{cfg.ano}</p>
            </div>
        </div>""")
        self._nova_pagina()
        ...
        # Página do Sumário
        self._coletar_entradas_sumario(self.doc_abnt.estrutura_textual)
        self._renderizar_sumario_html()
        self._nova_pagina()
    ...
    def _coletar_entradas_sumario(self, no_pai: Capitulo, prefixo_numeracao=""):
        for i, no_filho in enumerate(no_pai.filhos, 1):
            ...
            self.entradas_sumario.append({
                "numero": numero_completo, "titulo": no_filho.titulo,
                "nivel": nivel, "id_ancora": f"secao-{numero_completo.replace('.', '-')}"
            })
            self._coletar_entradas_sumario(no_filho, f"{numero_completo}.")

    def _renderizar_sumario_html(self):
        self._adicionar_elemento("<h1>SUMÁRIO</h1>", ALTURA_TITULO_SECAO)
        for entrada in self.entradas_sumario:
            titulo = entrada["titulo"].upper() if entrada["nivel"] == 1 else entrada["titulo"]
            item_html = f"""
            <p class="sumario-item sumario-nivel-{entrada['nivel']}">
                <a href="#{entrada['id_ancora']}">
                    {entrada['numero']} {titulo}
                </a>
            </p>"""
            self._adicionar_elemento(item_html, ALTURA_LINHA_TEXTO)
    ...
    def _renderizar_secoes_recursivamente_html(self, no_pai: Capitulo, prefixo_numeracao=""):
        for i, no_filho in enumerate(no_pai.filhos, 1):
            ...
            self._adicionar_elemento(f"<h1 id='{id_ancora}'>{titulo_texto}</h1>", ALTURA_TITULO_SECAO)
            ...
            if no_filho.conteudo:
                ...
                for k, parte in enumerate(partes_conteudo):
                    if k % 3 == 0:
                        ...
                        tipo, titulo = parte, partes_conteudo[k+1]
                        if tipo == "Tabela":
